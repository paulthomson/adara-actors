...
//            mainTask.Wait();
            ...
            for (int i = 1; i <= 1000000; ++i)
            {
                ...
                {
                    var mainMailbox = runtime2.CurrentMailbox();

                    IMailbox<object> actor1 =
                        runtime2.Create(() =>
                        {
                            IMailbox<object> actor1Mb =
                                runtime2.CurrentMailbox();

                            object msg = actor1Mb.Receive();
                            Console.WriteLine(msg);

                            IMailbox<object> actor1_2 =
                                runtime2.Create(() =>
                                {
                                    IMailbox<object> actor1_2Mb =
                                        runtime2.CurrentMailbox();
                            ...
                            msg = actor1Mb.Receive();
                            Console.WriteLine(msg);
                            mainMailbox.Send("ok1");
                            return 1;
                        });

                    IMailbox<object> actor2 =
                        runtime2.Create(() =>
                        {
                    ...
                    object m = mainMailbox.Receive();
                    Console.WriteLine(m);
                    m = mainMailbox.Receive();
                    Console.WriteLine(m);

                    testingRuntime.WaitForDeadlock();
